            ...
            >
                ...
                >
                    ...
                    <div class="p-3 w-full m-5 border rounded-3xl shadow-lg">
                        ...
                        <div class="flex justify-end items-center py-1 text-sm">
...
async function handleFileUploadFromGroup(docs) {
    isLoading.value = true; // ✅ 로딩 시작
    ...
    for (const doc of docs) {
        if (!doc._id) continue; // ✅ ID가 없는 경우 건너뛰기

        try {
            const formData = new FormData();
            formData.append("file", new File([""], doc.name)); // ✅ 빈 파일 객체 생성
            formDataArray.push(formData);
        } catch (err) {
            console.error("❌ 문서 그룹 파일 처리 중 오류 발생:", err);
        }
    }
